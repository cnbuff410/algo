/* Min priority queue implementation for huffman code Node */
package main
...
func NewMinPQ(capacity int) *minPQ {
	list := make([]Node, capacity+1) // Start from 1
	count := 0
...
func (pq *minPQ) Sink(k int) {
	for k * 2 <= pq.count {
		j := k * 2
		if (j < pq.count && pq.list[j].freq > pq.list[j+1].freq) { j++ }
		if !(pq.list[k].freq > pq.list[j].freq) { break }
...
func (pq *minPQ) Swim(k int) {
	for k > 1 && pq.list[k/2].freq > pq.list[k].freq {
		pq.list[k/2], pq.list[k] = pq.list[k], pq.list[k/2] // swap
		k = k / 2
	}
}

func (pq *minPQ) Insert(v Node) {
